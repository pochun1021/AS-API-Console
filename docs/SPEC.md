...
- 欄位語意：
  ...
  - `expires_at` 為目前有效到期時間；extend 時需依 key 是否已 expired 分流計算：
    ...
    - `expired` key：需先將 `application_date` 改寫為 `extend_action_date`，再以 `new_expires_at = application_date + original_duration_days` 重新計算
...
- 規則：
  ...
  - 呼叫 provider `update` 時，request body 需以 `key` 欄位傳送舊明文 key，其餘限制欄位沿用 `generate` wire format；`duration` 需送「目前有效起算日到新 `expires_at` 的總天數」：
    - `active` key：`provider_duration_days = (new_expires_at.date() - application_date).days`
...
17. 外部 provider 驗證 header 需使用 `Authorization: Bearer {PROVIDER_MASTER_KEY}`；`update`、`block` 若需舊明文 key，request body 一律以 `key` 欄位傳送；`update` 用於 extend 或 alias 同步時可帶 `key_alias`，且 `duration` 一律需與當前 key 的目標有效期一致：對 extend 為「當前有效起算日到新 `expires_at` 的總天數」；其中 `active` key 的起算日為原 `application_date`，`expired` key 的起算日為本次 `extend_action_date`（並需同步寫回 `application_date`）；對 alias 同步也需沿用同一口徑，避免覆蓋既有效期；`generate` 成功時一律自 response `key` 讀取新明文 secret。external provider mode 缺少 `PROVIDER_TEAM_ID` 時，`applications`、`renew`、`limit-strategy-config` 同步必須 fail fast。
...
26. 一般使用者可展延本人 `active|expired` key；展延 `revoked` key 時需回傳 `KEY_NOT_EXTENDABLE`。`active|expired` key 皆可隨時展延。extend 成功後：
  ...
  - provider `duration` 需送目前有效起算日到新 `expires_at` 的總天數，不得直接等同本地 `duration_months`，也不得再使用 `remaining_days` 模型。
