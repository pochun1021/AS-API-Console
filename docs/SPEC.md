...
- 時間欄位語意：`application_date` 為原始申請日；`duration_months` 為目前這把 key 已累計生效的總月數（原申請月數 + 每次成功 extend 的月數）；`expires_at` 為目前有效到期時間。
...
- 詳情視窗需沿用 API 時間欄位語意，不得把曾展延 key 的 `application_date`、`duration_months`、`expires_at` 顯示成彼此矛盾的資訊；`application_date` 為原始申請日，`duration_months` 為累計總月數，`expires_at` 為目前有效到期時間。
- 一般使用者僅可查本人資料。
...
- 欄位語意：
  ...
  - `application_date` 為原始申請日期；extend 不得改寫此欄位
  - `duration_months` 為目前此 application/key 所代表的累計有效月數；初次核發時為原申請月數，每次 extend 成功後需累加該次展延月數
...
- 讀取欄位語意：`application_date` 為原始申請日；`duration_months` 為目前此 key 已累計生效的總月數（非最近一次 extend request 值）；`expires_at` 為目前有效到期時間。
...
- 讀取欄位語意：`application_date` 為原始申請日；`duration_months` 為目前此 key 已累計生效的總月數（原申請月數 + 每次成功 extend 的月數）；`expires_at` 為目前有效到期時間。
...
21. `GET /main/api/v1/api-keys` 與 `GET /main/api/v1/api-keys/{id}` 的 `application_date`、`duration_months`、`expires_at` 語意需一致：`application_date` 為原始申請日、`duration_months` 為累計總月數、`expires_at` 為目前有效到期時間；frontend 清單與詳情不得把曾展延 key 顯示成互相矛盾的時間資訊。
...
26. 一般使用者可展延本人 `active|expired` key；展延 `revoked` key 時需回傳 `KEY_NOT_EXTENDABLE`。`active` key 僅能在 `expires_at - now(UTC) <= 30 days` 時展延，且此門檻同時適用於 `user` 與 `admin`；超過 30 天時需回傳 `KEY_EXTEND_NOT_NEAR_EXPIRY`。`expired` key 不受此限制。extend 成功後，`application_date` 仍為原申請日，讀取 API 的 `duration_months` 需改為累計總月數；若 key 已過期，新的 `expires_at` 需以 extend 成功當下為基準計算。
