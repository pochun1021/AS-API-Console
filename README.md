...
目前已登入的 `user` 與 `admin` 可透過「服務使用說明」頁查看 provider 提供的可用模型清單，以及 repo 內維護的服務串接說明與 Python 範例；也可透過 `/usage` 查看指定 API Key 在自訂日期區間內的每日 token / spend 使用量。系統公告集中顯示於「系統公告」頁，並以表格列示，點擊標題後以彈窗顯示公告內文，而「服務使用說明」固定常駐於該頁。登入後首頁預設導向「系統公告」頁。詳細契約與頁面行為以 `docs/SPEC.md` 為準。
