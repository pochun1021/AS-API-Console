...
目前已登入的 `user` 與 `admin` 可透過「服務使用說明」頁查看 provider 提供的可用模型清單，以及 repo 內維護的服務串接說明與 Python 範例；登入後共用版型另提供系統公告區塊與固定「服務使用說明」提示卡。詳細契約與頁面行為以 `docs/SPEC.md` 為準。
...
- `System Announcements Page`：`server-side table`，僅 `admin` 可管理公告 CRUD。
- `Institute View Page`：`local-full-dataset table`。
- `Service Usage Guide Page`：登入後的 `user` 與 `admin` 都可使用，整合模型清單與服務串接說明。
- `System Announcement Surface`：登入後共用版型顯示有效公告，並固定提供連往 `/usage-examples` 的服務使用說明提示卡。
...
- 系統公告由 `admin` 管理；`user` 與 `admin` 都只能查看目前有效公告。
- 固定「服務使用說明」提示卡永遠連到 `/usage-examples`，不依賴公告是否存在。
