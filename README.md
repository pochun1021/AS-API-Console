...
目前已登入的 `user` 與 `admin` 可透過「服務使用說明」頁查看 provider 提供的可用模型清單，以及 repo 內維護的服務串接說明與 Python 範例；登入後共用版型在存在有效公告時，會顯示系統公告區塊，而「服務使用說明」固定常駐於「系統公告」頁。登入後首頁預設導向「系統公告」頁。詳細契約與頁面行為以 `docs/SPEC.md` 為準。
...
- `System Announcements Page`：`user` 與 `admin` 都可進入，且主導覽列入口位於最左側。
  - `user` 僅可查看目前有效公告；`admin` 可管理公告 CRUD，管理列表仍為 `server-side table`。
- `Institute View Page`：`local-full-dataset table`。
- `Service Usage Guide Page`：登入後的 `user` 與 `admin` 都可使用，整合模型清單與服務串接說明。
- `System Announcement Surface`：登入後共用版型在有有效公告時顯示公告區塊，並一併提供連往 `/usage-examples` 的服務使用說明提示卡。
...
- 「服務使用說明」提示卡連到 `/usage-examples`，並僅在有有效公告時於公告區塊內顯示。
...
- `active|expired` key 皆可隨時 `extend`；`revoked` key 不可 `extend`。
- 讀取 API 的時間欄位語意固定為：`application_date`=最近一次申請或展延後的起算日、`duration_months`=目前這一輪有效期時長、`expires_at`=目前有效到期時間。
- `expires_at` 採 fixed-day 規則：`1|6|12` 個月固定視為 `30|180|360` 天。初次核發與後續每次 `extend` 都以當次起算日重新計算 `expires_at = application_date + effective_duration_days`；`duration_months` 不做累加。
...
- `extend` 呼叫 `POST /key/update` 時，`duration` 一律需送「從 `api_keys.created_at` 起算到新 `expires_at` 的總天數」
