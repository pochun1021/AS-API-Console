...
- 讀取 API 的時間欄位語意固定為：`application_date`=原始申請日、`duration_months`=目前累計有效月數、`expires_at`=目前有效到期時間。
- `expires_at` 採 fixed-day 規則：`1|6|12` 個月固定視為 `30|180|360` 天。初次核發時以 `application_date + original_duration_days` 取得基準到期日；後續 `extend` 時以 `extend_action_date` 相對 `application_date` 的天數位移推導新的 `expires_at`。例如 2026-06-01 申請 1 個月，基準到期日為 2026-07-01；若於 2026-06-10 `extend`，新的到期日為 2026-07-10。
...
- `extend` 呼叫 `POST /key/update` 時，`duration` 一律需送「從原始 `application_date` 起算到新 `expires_at` 的總天數」
