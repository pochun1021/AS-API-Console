...
目前未登入使用者也可透過 `/` 或 `/usage-examples` 查看 repo 內維護的「服務使用說明」與 Python 範例，並可在頁面上切換中英文；點選登入時才會依上線狀態決定是否先顯示 `/login-coming-soon`。登入成功後預設導向「系統公告」頁，登出後回到未登入公開首頁。已登入的 `user` 與 `admin` 可在 `/usage-examples` 額外查看 provider 提供的可用模型清單，也可透過 `/usage` 查看指定 API Key 在自訂日期區間內的每日 token / spend 使用量。系統公告集中顯示於「系統公告」頁，並以表格列示，點擊標題後以彈窗顯示公告內文，而「服務使用說明」固定常駐於該頁；主導覽列中的「系統公告」與「服務使用說明」維持相鄰共享入口，且在小螢幕下需改用 responsive 導覽而非擁擠的單列按鈕。詳細契約與頁面行為以 `docs/SPEC.md` 為準。

## 文件入口
- 產品/API/data 契約：`docs/SPEC.md`
- 服務使用說明文件：`docs/service-usage-guide.zh-TW.md`、`docs/service-usage-guide.en.md`
...
- `Usage Page`：依 API Key 與日期區間查看每日使用量圖表。
...
- `System Announcements Page`：`user` 與 `admin` 都可進入，且主導覽列入口位於最左側。
  - `user` 僅可查看目前有效公告，並可用頁面外搜尋欄位查標題；`admin` 可管理公告 CRUD，管理列表仍為 `server-side table`。
  - `user` 與 `admin` 都以表格查看公告，點擊標題後才以彈窗查看公告內文。
- `Institute View Page`：`local-full-dataset table`。
- `Service Usage Guide Page`：登入後的 `user` 與 `admin` 都可使用，整合模型清單與服務串接說明。
- `Operation Audit Logs Page`、`Auth Audit Logs Page`：`server-side table`。
  - `Operation Audit Logs Page` 規格包含既有操作稽核、登入稽核，以及規劃中的 `Scheduler Logs` 第三個 tab。
- 共用主導覽列：桌機維持頂部導覽列；較小寬度需切換為 hamburger menu + Drawer，避免導覽文字在窄畫面擠壓。
...
- 系統公告由 `admin` 管理；`user` 與 `admin` 都可從主導覽列進入系統公告頁查看目前有效公告。
- 「服務使用說明」提示卡固定顯示於系統公告頁，連到 `/usage-examples`；同一路由在未登入時會顯示公開靜態說明文件，不顯示模型清單。
...
- `extend` 僅允許 `active` key；`expired|revoked` key 不可 `extend`。
- 讀取 API 的時間欄位語意固定為：`application_date`=最近一次申請或展延後的起算日、`duration_days`=目前這一輪有效期時長、`expires_at`=目前有效到期時間。
- `expires_at` 採 fixed-day 規則：生效時長固定僅允許 `30|180|360` 天。初次核發與後續每次 `extend` 都以當次起算日重新計算 `expires_at = application_date + effective_duration_days`；`duration_days` 不做累加。
...
- `extend` 呼叫 `POST /key/update` 時，`duration` 一律直接送該 key 的 `original_duration_days` 對應值（`30d|180d|360d`），不再依 `api_keys.created_at` 累算總天數
...
- 前端公開錯誤頁：`/main/login-error` 用於登入流程非預期失敗，顯示失敗路由、簡短原因與 `request_id`，並提供返回 `/main/login` 重新登入。
...
- Vite dev server 已設定 `/main/api` 與登入相關路徑（`/main/login`、`/main/auth`、`/main/logout`）proxy 到 `http://127.0.0.1:8000`（若需 `npm run dev` 分離開發可直接使用）。
...
- 提醒用途：提醒信僅供通知，不作為 `extend` eligibility 的判定依據；`extend` 是否允許仍以 key 當下有效狀態判定，僅 `active` 可展延。
